...
public class CliParser<T> where T : new()
{
    ...
    public T Parse(string[] args) 
    {
        ...
        {
            if (!_argMap.TryGetValue(args[i], out var prop))
            {
                continue;
            }
            if (i + 1 >= args.Length)
            {
                throw new CliParserException($"Expected keyword arg, but got none at end");
            }
            var value = args[i + 1];
    ...
    /// </summary>
    /// <returns>Dictionary(string, PropertyInfo) -- mappings of argument Property name to the corresponding
